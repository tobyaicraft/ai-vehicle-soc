...
class ControlPanel:
    ...
    def _build_ui(self):
        ...
        for text, key_name in [("U", "u"), ("I", "i"), ("SPC", "space"), ("P", "p")]:
        ...
        # Chart
        chart_frame = ttk.Frame(right_col, style="Dark.TFrame")
        chart_frame.pack(fill=tk.BOTH, expand=True)

        self.fig = Figure(figsize=(6, 2.5), dpi=100, facecolor=CHART_BG)
        self.ax = self.fig.add_subplot(111)
        self._setup_chart()
        self.canvas = FigureCanvasTkAgg(self.fig, master=chart_frame)
        self.canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)

        # Sensor panels: Left IR | Car Visual | Right IR
        sensor_bottom = ttk.Frame(right_col, style="Dark.TFrame")
        sensor_bottom.pack(fill=tk.BOTH, expand=True, pady=(5, 0))

        # Left IR
        lp = ttk.Frame(sensor_bottom, style="Dark.TFrame", width=160)
        lp.pack(side=tk.LEFT, fill=tk.Y)
        lp.pack_propagate(False)
        ttk.Label(lp, text="LEFT IR", foreground=ACCENT_L,
                  background=BG_COLOR, font=("Segoe UI", 11, "bold")).pack(pady=(5, 0))
        ...
        self.car_canvas = tk.Canvas(center, bg=CHART_BG, highlightthickness=0, height=250)
        self.car_canvas.pack(fill=tk.BOTH, expand=True)

        # Right IR
        rp = ttk.Frame(sensor_bottom, style="Dark.TFrame", width=160)
    ...
    def _setup_chart(self):
        ...
        self.ax.tick_params(colors=FG_COLOR, labelsize=7)
        ...
        self.line_us, = self.ax.plot([], [], color=ACCENT_U, linewidth=2,
    ...
    def _draw_car(self, left_dist, right_dist, us_dist):
        ...
        # Ultrasonic
        us_len = max(15, min(80, us_dist * 0.6))
        us_color = DANGER_COLOR if us_dist < 20 else (
            WARN_COLOR if us_dist < 50 else ACCENT_U)
        front_y = cy - car_h // 2
        c.create_polygon(cx - 15, front_y, cx + 15, front_y,
                         cx + 35, front_y - us_len, cx - 35, front_y - us_len,
                         fill="", outline=us_color, width=2)
        c.create_text(cx, front_y - us_len - 14,
                      text=f"{us_dist:.0f} cm", fill=us_color,
                      font=("Consolas", 12, "bold"))

        # IR Left
        ir_angle = math.radians(45)
        left_len = max(12, min(70, left_dist * 0.8))
        ...
        # IR Right
        right_len = max(12, min(70, right_dist * 0.8))
